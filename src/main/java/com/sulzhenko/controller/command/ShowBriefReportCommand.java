...
public class ShowBriefReportCommand implements Command {
    private static final Logger logger = LogManager.getLogger(ShowBriefReportCommand.class);
    @Override
    public String execute(HttpServletRequest request, HttpServletResponse response) {
        setPage(request);
        setMenu(request);
        UserActivityService userActivityService = getApplicationContext().getUserActivityService();
        List<UserActivityDTO> userActivities = userActivityService.listUserActivitiesBriefSorted(request);
